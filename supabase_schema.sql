...
create table if not exists public.requests (
    ...
    details text,
    status text default 'pending' check (status in ('pending', 'assigned', 'completed')) not null,
    assigned_senior_id uuid references public.profiles(id) on delete set null,
    created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
...
returns trigger as $$
begin
  insert into public.profiles (id, name, email, avatar_url, role, verification_status)
  values (
    new.id,
...
-- 3. REQUESTS POLICIES
create policy "Requests select: own, assigned senior, or admin" on public.requests
    for select using (
        student_id = auth.uid()
        or assigned_senior_id = auth.uid()
        or public.is_admin(auth.uid())
    );

create policy "Requests insert: own only" on public.requests
    for insert with check (student_id = auth.uid());

create policy "Requests update: own or admin" on public.requests
    for update using (
        student_id = auth.uid() 
        or public.is_admin(auth.uid())
    );

create policy "Requests delete: admin only" on public.requests
